...
void	verb_adv(t_vm *vm, t_process *process)
{
	int		len;
	int		i;
	...
	if (process->pc == 0)
		ft_putstr("0x");
	ft_printf("%#.4x (%d) -> ", process->pc, process->pc);
	if (process->pc + len == 0)
		ft_putstr("0x");
	ft_printf("%#.4x (%d)) ", process->pc + len, process->pc + len);
	while (i < len)
	{
...
int ld(t_vm *vm, t_op *op, t_process *process)
{
	...
	if (op->params[0] == 0)
		...
		process->carry = 0;
	ft_printf("register : %d       avant : %ld       ", op->params[1], process->registre[op->params[1] - 1]);    
	if (op->params[1] < REG_NUMBER)
